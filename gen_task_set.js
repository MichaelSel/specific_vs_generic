...
async function gen_task_set (sub_id,prefix="SVG",root='./task_sets') {
        ...
        /**Experiment 1*/
        // const stimuli_diatonic = make_stimuli(sub_name,25,4,[0,2,4,5,7,9,11]) /**Set Settings here*/

            // const stimuli_lyd7b = make_stimuli(sub_name,5,5,[0,2,4,6,7,9,10]) /**Set Settings here*/

        /**Experiment 2*/
        const stimuli_harmonic = make_stimuli(sub_name,10,4,[0,2,3,5,7,9,11]) /**Set Settings here*/
        const stimuli_octa = make_stimuli(sub_name,10,4,[0,2,3,5,6,8,9]) /**Set Settings here*/
        const stimuli_harmonic_major = make_stimuli(sub_name,10,4,[0,2,4,5,7,8,11]) /**Set Settings here*/
    ...
    //                     return e}) /**Make big array with all kinds of diatonic conditions and shuffle it*/
        ...
        const stimuli = edo.shuffle_array([].concat(stimuli_harmonic).concat(stimuli_octa).concat(stimuli_harmonic_major))
                .map((stimulus,Q_num)=>{
